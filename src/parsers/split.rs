...
pub fn parse_markup_block(pair: Pair<Rule>) -> Result<String, pest::error::Error<Rule>> {
    Ok(pair
        .into_inner()
...
pub fn parse_src_block(pair: Pair<Rule>) -> Result<Content, pest::error::Error<Rule>> {
...
fn parse_source_comment(pair: Pair<Rule>) -> Result<String, pest::error::Error<Rule>> {
    Ok(match pair.as_rule() {
        Rule::source_comment => {
...
pub fn parse_code_placeholder_block(pair: Pair<Rule>) -> Result<Content, pest::error::Error<Rule>> {
    Ok(match pair.as_rule() {
        Rule::source_comment_block => Content::Code {
            code: pair
                .into_inner()
                .map(parse_source_comment)
                .collect::<anyhow::Result<Vec<String>, pest::error::Error<Rule>>>()?
                .join("\n"),
        },
        Rule::markup_block => Content::Markup {
            markup: parse_markup_block(pair)?,
...
pub fn parse_inner_block(pair: Pair<Rule>) -> Result<Inner, pest::error::Error<Rule>> {
    let r = pair.as_rule();
    Ok(match r {
        Rule::code_block => {
            ...
            let placeholder: Vec<Content> = placeholder_pair
                ...
                .collect::<anyhow::Result<Vec<Content>, pest::error::Error<Rule>>>()?;
...
pub fn parse_attribute(pair: Pair<Rule>) -> Result<(String, String), pest::error::Error<Rule>> {
    Ok(match pair.as_rule() {
        Rule::attr => {
...
pub fn parse_value(pair: Pair<Rule>) -> Result<Value, pest::error::Error<Rule>> {
    Ok(match pair.as_rule() {
        Rule::block => {
            let mut block_segments = pair.into_inner();
            ...
            let attributes_vec = attribute_pairs
                .into_inner()
                .map(parse_attribute)
                .collect::<anyhow::Result<Vec<(String, String)>, pest::error::Error<Rule>>>()?;
            let content = content_pairs
                .into_inner()
                .map(parse_inner_block)
                .collect::<anyhow::Result<Vec<Inner>, pest::error::Error<Rule>>>()?;
...
pub fn parse_code_string(content: &str) -> Result<CodeTaskDefinition, pest::error::Error<Rule>> {
    ...
    let vals = p
        .into_iter()
        .map(parse_value)
        .collect::<anyhow::Result<Vec<Value>, pest::error::Error<Rule>>>()?;
